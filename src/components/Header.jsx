...
export const Header = ({ onSignOut, onExport, onToggleSettings, user, profile, syncSummary, isOnline }) => {
  ...
  return (
    <header className="header">
      <div className="brand">
        ...
        <div>
          <h1>Clipboard</h1>
          <p>Secure clipboard intelligence and sync.</p>
        </div>
      </div>
      <div className="account-pill" aria-label="Logged in account">
        ...
        <div className="account-meta">
          ...
          <span>{email}</span>
          <small className="account-status">{isOnline ? 'Online' : 'Offline'} | {syncSummary}</small>
        </div>
      </div>
